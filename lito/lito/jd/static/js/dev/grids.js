...
//idtr, idtramite, tramite, idrequisito, requisito, plazointerno, plazolegal
const xhttp = new XMLHttpRequest();
xhttp.onload = function () {
    var json_records = JSON.parse(this.responseText);
    Array.from(json_records).forEach(function(element){
        orderList.add({
            idtr: '<a href="apps-ecommerce-order-details.html" class="fw-medium link-primary">#'+element.idtr+'</a>',
            ...
            plazointerno: element.plazointerno,
            plazolegal: element.plazolegal,
            status: isStatus(element.estado)

        });
        orderList.sort('idtr', { order: "desc" });
        refreshCallbacks();
    });
    orderList.remove("idtr", `<a href="apps-ecommerce-order-details.html" class="fw-medium link-primary">#</a>`);
}
xhttp.open("GET", "treq");
xhttp.send();

isCount = new DOMParser().parseFromString(
    orderList.items.slice(-1)[0]._values.idtr,
    "text/html"
);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Editar Trámite";
...
function isStatus(val) {
    switch (val) {
        case "A":
            return ('<span class="badge badge-soft-success text-uppercase">Activo</span>');        
        case "I":
            return ('<span class="badge badge-soft-success text-uppercase">Inactivo</span>'); 
        case "Delivered":
            return (
